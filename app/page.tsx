...
export default function Home() {
  ...
  function buildError(data: any, fallback: string) {
    return [data?.error || fallback, data?.hint, data?.detail].filter(Boolean).join("\n\n");
  }
  ...
  async function refreshProjects() {
    try {
      ...
      if (!response.ok) throw new Error(buildError(data, "프로젝트 목록 오류"));
      setProjects(data.projects || []);
      setSaveStatus("프로젝트 DB 연결됨");
    } catch (error) {
      setSaveStatus("DB 연결 확인 필요");
      if (error instanceof Error && !result) setResult(error.message);
    }
  }
  ...
  async function ensureProject() {
    ...
    const data = await response.json();
    if (!response.ok) throw new Error(buildError(data, "프로젝트 생성 실패"));
  ...
  async function saveStage(id: number, analysisType: string, payload: unknown) {
    try {
      ...
      if (!response.ok) throw new Error(buildError(data, "자동 저장 실패"));
  ...
  async function loadProject(id: number) {
    ...
    try {
      ...
      const data = await response.json();
      if (!response.ok) throw new Error(buildError(data, "프로젝트 불러오기 실패"));
  ...
  async function analyze(mode: Mode) {
    ...
    try {
      ...
      const data = await response.json();
      if (!response.ok) throw new Error(buildError(data, "분석 실패"));
  ...
  async function createStrategyInterview(){if(!analysisContext){setResult("먼저 사업분석 또는 요구사항 추출을 실행해주세요.");return;}setLoading(true);try{const id=await ensureProject();const response=await fetch("/api/strategy",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({projectName,analysisContext})});const data=await response.json();if(!response.ok)throw new Error(buildError(data,"전략 인터뷰 생성에 실패했습니다."));setQuestions(data.result.questions||[]);setResult(JSON.stringify(data.result,null,2));await saveStage(id,"strategy_interview",data.result);}catch(error){setResult(error instanceof Error?error.message:"알 수 없는 오류가 발생했습니다.");}finally{setLoading(false);}}
  async function createWinThemes(){if(!questions.length)return;setLoading(true);try{const id=await ensureProject();const response=await fetch("/api/win-theme",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({projectName,analysisContext,questions,answers})});const data=await response.json();if(!response.ok)throw new Error(buildError(data,"Win Theme 생성에 실패했습니다."));setThemes(data.result.themes||[]);setSelectedTheme(data.result.recommended_theme_id||"");setResult(JSON.stringify(data.result,null,2));await saveStage(id,"win_theme",data.result);}catch(error){setResult(error instanceof Error?error.message:"알 수 없는 오류가 발생했습니다.");}finally{setLoading(false);}}
  async function createSectionMessages(){if(!selectedTheme)return;setLoading(true);try{const id=await ensureProject();const response=await fetch("/api/section-message",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({projectName,analysisContext,questions,answers,selectedTheme,themes})});const data=await response.json();if(!response.ok)throw new Error(buildError(data,"Section Message 생성에 실패했습니다."));setClaims(data.result.claims||[]);setMatrix(data.result.requirement_matrix||[]);setSections(data.result.sections||[]);setResult(JSON.stringify(data.result,null,2));await saveStage(id,"section_message",data.result);}catch(error){setResult(error instanceof Error?error.message:"알 수 없는 오류가 발생했습니다.");}finally{setLoading(false);}}

  return <main className="shell"><header className="hero"><div><p className="eyebrow">PROPOSAL AI · PROTOTYPE 0.5</p><h1>전시·체험사업 제안서 AI</h1><p>프로젝트 단위로 RFP 분석부터 전략·메시지까지 누적 저장합니다.</p></div><span className="status">V0.5</span></header><section className="panel projectBar"><div className="outputHead"><h2>프로젝트</h2><span>{saveStatus}</span></div><div className="projectList"><button onClick={resetWorkspace}>+ 새 프로젝트</button>{projects.map(p=><button key={p.id} className={projectId===p.id?"selectedProject":"secondary"} onClick={()=>void loadProject(p.id)}>{p.project_name}</button>)}</div></section><section className="grid"><div className="panel"><h2>1. 프로젝트와 발주자료</h2><label>사업명<input value={projectName} onChange={e=>setProjectName(e.target.value)}/></label><div className="row"><label>사업예산<input value={budget} onChange={e=>setBudget(e.target.value)}/></label><label>사업면적<input value={area} onChange={e=>setArea(e.target.value)}/></label></div><label>RFP / 과업지시서 PDF<input className="fileInput" type="file" accept="application/pdf" onChange={e=>setFile(e.target.files?.[0]||null)}/></label>{file&&<p className="fileName">선택 파일: {file.name}</p>}<label>또는 텍스트 입력<textarea value={rfpText} onChange={e=>setRfpText(e.target.value)} placeholder="PDF가 없을 때 RFP 내용을 붙여넣을 수 있습니다."/></label><div className="actions"><button disabled={loading} onClick={()=>analyze("project")}>사업 분석</button><button className="secondary" disabled={loading} onClick={()=>analyze("requirements")}>요구사항 추출</button><button className="accent" disabled={loading||!analysisContext} onClick={createStrategyInterview}>전략 인터뷰</button></div></div><div className="panel output"><div className="outputHead"><h2>2. AI 작업 결과</h2>{loading&&<span>분석 중…</span>}</div>{result?<pre>{result}</pre>:<div className="empty">프로젝트를 만들고 RFP 분석을 시작하세요. 결과는 자동 저장됩니다.</div>}</div></section>
  {questions.length>0&&<section className="panel interview"><h2>3. 전략 인터뷰</h2>{questions.map(q=><div className="question" key={q.id}><strong>{q.id}. {q.question}</strong>{q.reason&&<p>{q.reason}</p>}{q.options?.length?<div className="chips">{q.options.map(o=><button type="button" className="chip" key={o} onClick={()=>setAnswers(p=>({...p,[q.id]:o}))}>{o}</button>)}</div>:null}<textarea className="answer" value={answers[q.id]||""} onChange={e=>setAnswers(p=>({...p,[q.id]:e.target.value}))}/></div>)}<button className="accent" disabled={loading} onClick={createWinThemes}>Win Theme 생성</button></section>}
  ...
  <section className="roadmap"><strong>Workflow</strong><span className="done">프로젝트 DB</span><b>→</b><span>RFP 분석</span><b>→</b><span>전략 인터뷰</span><b>→</b><span>Win Theme</span><b>→</b><span>Section Message</span><b>→</b><span>자동 리서치</span><b>→</b><span>Concept / 예산</span><b>→</b><span>Blueprint</span></section></main>;
}
